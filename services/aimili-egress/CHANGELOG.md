...
## [Custom 1.2.0] - 2026-09-10

### 新增 (Added)

- 原生管理网页新增“现有国家”和“补充国家”两个下拉框，以及“补充所选国家”按钮，直接复用已有的后台单国刷新状态机。
- 主界面新增 VPNGate 官方节点/国家总数与当前有效节点/国家数统计，并实时显示单国补充阶段、检测数量和完成结果。

### 变更 (Changed)

- 网页节点快照严格只返回 `probe_status=available` 的节点，移除失效节点筛选入口；升级启动时会清理旧版本遗留的失效和未验证节点。
- 手动连接、单节点/批量复测或主出口健康检查淘汰节点后，会触发一次去重的后台有效池补充；失败节点进入冷却，不会立即反复测试。

### 测试 (Tests)

- 新增网页安全快照、统计计数、启动清理、后台补池去重和原生 UI 契约测试。

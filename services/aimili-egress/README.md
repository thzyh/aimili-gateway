...
- 运行数据不放进 Git。为了兼容现有服务器，示例 unit 继续使用 `/opt/aimilivpn/vpngate_data`，源码则从 `/opt/aimili-gateway/services/aimili-egress` 启动。
- 原 `aimili-vpngate` 仓库继续保留历史和兼容用途，但不再作为双向开发源；新功能只在本目录修改。

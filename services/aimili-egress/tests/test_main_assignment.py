...
class ManagerMainAssignmentTests(unittest.TestCase):
    ...
    def test_slot_facades_remain_independent_during_pending_main_assignment(self):
        with (
            mock.patch.object(
            ...
            mock.patch.object(
                manager.main_assignment_coordinator,
                "assignment_action_allowed",
                return_value=True,
            ),
            mock.patch.object(manager, "read_nodes", return_value=[]),
            mock.patch.object(manager, "delete_slot", return_value={"ok": True}),
            mock.patch.object(manager, "set_slot_country"),
            mock.patch.object(manager, "set_slot_type"),
            mock.patch.object(manager, "set_slot_isp"),
            mock.patch.object(manager, "set_slot_pin"),
            mock.patch.object(manager.egress_repair_store, "clear"),
        ):
            self.assertNotEqual(manager.create_managed_slot("JP", "datacenter").get("error_code"), "operation_busy")
            self.assertNotEqual(manager.assign_managed_slot(0, "node", "JP", "datacenter").get("error_code"), "operation_busy")
            self.assertTrue(manager.delete_managed_slot(0)["ok"])

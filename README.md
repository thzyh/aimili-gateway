...
故障恢复采用“每个活动连接一个专属热备用”模型：主连接和每个未暂停的普通出口各占一个保护目标，热备用数量随活动目标自动变化，暂停的出口不会虚报备用。备用拨号按原国家优先、住宅优先、机房兜底、必要时跨国；故障确认、备用巡检、单轮候选数、并发拨号、退避、候选冷却和最终恢复预算均可在高级设置修改。提升热备用后，后台继续为同一目标补回备用；预算耗尽进入人工重试状态，不会冷替换正在工作的其他出口。国家明细同时显示可拨号数、近期真实出网通过数、住宅/机房数；IP 质量检测暂未实现，接口固定标记 `not_implemented`，不会伪造通过数量。

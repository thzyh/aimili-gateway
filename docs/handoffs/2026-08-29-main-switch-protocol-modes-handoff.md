...
> 历史状态说明：本文保留 2026-08-29 至 2026-09-03 的任务演进和当时事实。当前路径、Git、生产摘要与未完成事项请以 `docs/handoffs/2026-09-05-current-state-handoff.md` 为准，不要从本文早期“待设计/待部署”段落推断当前状态。
...
## 9. 2026-09-03 生产故障修复与存储整理续接结果

本节是本文前述历史状态之后的最新权威检查点；与旧协议矩阵或旧服务判断冲突时，以本节和生产最新只读检查为准。

- Gateway 修复提交 `d58956b` 已部署，涵盖普通失败可恢复操作、来源解析细分、统一 `UiNotice`、mixed-only 策略更新以及主连接在内的 3x-ui 漂移检测。
- AimiliVPN 的四个候选维护/拨号失败修复提交为 `4a6790f`、`9d57ca8`、`1a10ee5`、`4d3f254`，生产运行版本已包含这些修改。
- 清理后四项服务 `aimili-gateway`、`aimilivpn`、`x-ui`、`caddy` 均为 active，`NRestarts=0`；稳态为 1 个 Xray、4 个 OpenVPN。
- AimiliVPN 主连接 `active=true`、`egress_ok=true`；三个槽位均为 `up`、`egress_ok=true`，候选身份和出口身份与 Gateway DB 匹配。
- Gateway 当前四个逻辑出口全部 `ready`，协议矩阵为：主连接 XHTTP/REALITY、出口1 XHTTP/REALITY、出口2 Hysteria2/QUIC/TLS、出口3 XHTTP/REALITY。该矩阵不同于本文早期 Stage 5～9 的历史矩阵，属于 2026-09-03 最终只读事实，不能用旧记录覆盖。
- 3x-ui 漂移检测为公网 4、mixed 4、出站 4，`ownershipMatches=true`；没有调用“修复资源漂移”。非 Gateway 入站数量仍为 0，指纹保持 `4f53cda1…02b945`。
- 来源策略最终为 `enabled=true`、2 条 CIDR、`applyStatus=applied`。四个 mixed 各有 1 条允许规则（2 条用户 CIDR加 IPv4/IPv6 回环）、1 条兜底 blackhole 拒绝规则；策略事务内四个真实 SOCKS 出口均通过。
- 策略最终复核期间出现一次 x-ui 热应用控制连接重置，Gateway 正确进入 `repair_required`；x-ui 随即以有效配置恢复单 Xray。等待控制面稳定后重试同一 Gateway API 事务成功，未直接修改 Gateway DB 或 x-ui DB。
- 订阅当前包含四个逻辑出口；主连接复制、出口2替换、出口3双向协议切换已在本轮生产路径验证。内置浏览器没有现成 Gateway 登录会话，因此没有把服务器账户读入浏览器，也没有冒充“仅授权当前网络”按钮的人工点击已经通过。

VPS 精确清理结果：

- 删除 68 个经过绝对路径、边界、存在性、进程占用和保留目录重叠检查的历史目标，共释放 `3,006,283,776` 字节：历史 Gateway 回滚备份 `2,204,418,048` 字节、`/root` 旧部署目录 `299,008,000` 字节、`/tmp` 旧部署资产 `502,857,728` 字节。
- 删除项在 VPS 上不可恢复；保留三套已验证回滚基线：联合基线 `20260901T115959Z`、x-ui 基线 `20260902T090155Z-743607`、Gateway 基线 `20260903T030613Z-803955`。当前及保留数据库 `PRAGMA quick_check` 均为 `ok`，关键回滚二进制存在。
- `/tmp/aimili-egress-stage2` 在清理前后都不存在，没有再次删除。
- 最终根分区使用率为 54%，可用 `4,340,293,632` 字节；`/var/backups/aimili-gateway` 为 `236,077,798` 字节，`/tmp` 为 `1,066,494` 字节，`/root` 为 `112,211` 字节。

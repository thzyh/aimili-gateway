...
日期：2026-09-25（Asia/Shanghai）。状态：当前权威入口。较早章节只代表各自时间点。

## 2026-09-25 jjs 网页整项目更新

- 用户要求真实“检测 → 确认 → 更新进度”，并明确正式升级由用户亲自点击。只获准部署更新入口补丁，禁止代点 jjs 的 apply。
- 已在 jjs 原业务提交 `eea92cb` 上部署更新入口，运行标识 `dev+update-bootstrap`；没有提前部署 main 的业务改动。Gateway 已重启，其他三项服务启动时间与连接身份未变。
- 新实现为独立低权限下载器、root 离线验签安装器和持久事务；复用原请求客户端。服务端真实检测成功，前端 86 项、Go 全量、更新器 6 项测试通过。
- main 已有整项目更新实现，完整发布及最终验证以 [2026-09-25 更新记录](../verification/2026-09-25-project-update.md) 为准；禁止把测试结果表述成 jjs 已完成正式升级。
- 当前仅在 jjs 部署入口；没有给 xjp/bj 部署本次新更新功能。原有 `.tmp/`、`.test-gocache/` 与 `jp-*.ovpn` 未纳入 Git。

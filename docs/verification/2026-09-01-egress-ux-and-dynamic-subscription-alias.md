...
## 2026-09-03 生产故障修复与存储整理补充验证

### 本地实现

Gateway 提交 `d58956b` 完成以下最小修复：

- `degraded` 与真正的 `repair_required` 都提供“重新检测并同步”；普通候选失败或成功回滚不制造死状态。
- 来源识别区分无效直连端、非受信代理、缺失/无效/非公网转发地址；前端显示对应中文说明，不暴露内部错误码。
- 高级设置复用 `UiNotice`，成功、失败、处理中状态可区分且可关闭；保存后重新读取权威策略，失败时说明旧策略或回滚状态。
- 来源策略只更新四个 mixed 入站路由，不重写当前公网协议；漂移检测包含主连接以及四个公网、mixed、出站资源。
- 按生产事实把 3x-ui 的 Hysteria2 入站协议字段识别为 `hysteria`。

AimiliVPN 保留四个独立 TDD 修复提交：

- `4a6790f`：普通补池保护主连接、运行槽位和固定槽位候选。
- `9d57ca8`：明确 `No route to host` 的候选持久淘汰。
- `1a10ee5`：pending 槽位失败仍可重试，不伪造旧 pin 回滚失败。
- `4d3f254`：已诊断的远端连接超时按候选不可达淘汰。

代码提交前验证结果记录为：Gateway Go 全包 `-race`、`go vet`、前端 46 项测试、前端生产构建、scripts 95 项通过且 2 项 Windows symlink 条件跳过；AimiliVPN 162 项测试、`py_compile`；两个仓库 `git diff --check` 均通过。最终推送前应重新运行受影响范围验证，不能用本段历史结果替代新证据。

### 生产原操作与运行态

- Gateway 最终二进制经摘要、健康门和自动回滚安装器部署；AimiliVPN 四项修复此前已用相同健康门部署。
- 正式四出口检测通过；AimiliVPN 主连接 active/egress 正常，三个槽位均 up/egress 正常，身份与 Gateway DB 匹配。
- 主连接复制成功；出口2候选替换成功；出口3先切至 XHTTP 并完成真实连接，再切回 TCP/Vision成功。最终清理后的权威状态再次读取时，出口3当前为 XHTTP/REALITY；本记录不使用早期矩阵覆盖当前事实。
- 当前协议矩阵：主连接 XHTTP/REALITY、出口1 XHTTP/REALITY、出口2 Hysteria2/QUIC/TLS、出口3 XHTTP/REALITY。四项均为 `ready`、协议和订阅状态为 `ready`，公网和 SOCKS 延迟均已记录。
- 订阅包含四个当前逻辑出口；公网协议身份和关键参数匹配，动态中文别名生效。
- 3x-ui 漂移检测为公网 4、mixed 4、出站 4，`ownershipMatches=true`，警告已消失；没有点击“修复资源漂移”。非 Gateway 入站仍为 0，指纹保持 `4f53cda1…02b945`。
- 来源策略最终为 `enabled=true`、2 条 CIDR、`applied`；四个 mixed 各有 1 条允许规则、4 个来源（2 条用户 CIDR加双栈回环）和 1 条兜底拒绝规则。Gateway 事务中的四个真实 SOCKS 验证通过，公网协议矩阵未变化。
- 最终策略复核时首次热应用遇到 x-ui/Xray 控制连接重置并进入 `repair_required`；日志确认 x-ui 随即恢复单 Xray。控制面稳定后同一 Gateway API 事务重试成功，未直接写数据库。
- 最后 20 秒三次采样均为四服务 active、1 个 Xray、4 个 OpenVPN；四服务 `NRestarts=0`、`ExecMainStatus=0`。

内置浏览器没有现成 Gateway 登录会话，因此未执行“仅授权当前网络”按钮的真实浏览器点击，也未把服务器账户读取后注入浏览器。后端来源分类、Caddy 转发链路、策略事务、实际路由规则和前端文案已有自动化/生产证据，但不得将其表述为该按钮的人工 UI 点击证据。

### VPS 存储整理

- 清理前根分区 86%，可用约 `1,343,311,872` 字节；`/var/backups/aimili-gateway` 为 `2,440,495,104` 字节。
- 精确删除 68 个无进程占用的历史目标，共释放 `3,006,283,776` 字节：历史回滚备份 `2,204,418,048` 字节、`/root` 旧部署目录 `299,008,000` 字节、`/tmp` 旧上传/部署资产 `502,857,728` 字节。
- 删除前发现 2026-08-27 的三个 closing 会话仍引用 `/root/aimili-v1d-stage`；仅对核实后的 6 个精确 PID 发送 `SIGTERM`，全部退出，四服务保持 active。没有使用进程名通配终止其他进程。
- 删除项在 VPS 上不可恢复；保留三套验证过的回滚基线，当前与保留数据库均通过 `PRAGMA quick_check`，Gateway 与 x-ui 回滚二进制存在。
- `/tmp/aimili-egress-stage2` 本来就不存在，本次没有再次删除。
- 清理及辅助脚本移除后最终根分区 54%，可用 `4,340,293,632` 字节；`/var/backups/aimili-gateway` 为 `236,077,798` 字节，`/tmp` 为 `1,066,494` 字节，`/root` 为 `112,211` 字节。

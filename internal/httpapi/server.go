...
func NewServer(dependencies Dependencies) http.Handler {
	...
	mux.HandleFunc("GET /api/v1/settings/recovery", server.handleRecovery)
	mux.HandleFunc("PUT /api/v1/settings/recovery", server.handleRecovery)
	mux.HandleFunc("POST /api/v1/settings/aimilivpn/standbys/{index}/retry", server.handleRetryDedicatedStandby)

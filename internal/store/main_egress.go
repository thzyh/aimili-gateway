...
func (s *Store) SaveMainEgress(ctx context.Context, value MainEgress) error {
	if value.ResourceName != "agw-main" || len(value.CountryCode) != 2 || !value.ProxyType.Valid() || len(value.CandidateID) > 256 || value.PublicInboundID < 1 || value.MixedInboundID < 1 || value.PublicPort != 8443 || value.MixedPort < 1 || value.MixedPort > 65535 || value.UpdatedAt.IsZero() {
	...
	_, err := s.db.ExecContext(ctx, `INSERT INTO main_egress(id, resource_name, country_code, country_name, proxy_type, candidate_id, exit_ip, public_inbound_id, mixed_inbound_id, public_port, mixed_port, enabled, candidate_latency_ms, vless_latency_ms, socks_latency_ms, last_checked_at, last_error_code, updated_at)
		VALUES(1, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
		ON CONFLICT(id) DO UPDATE SET resource_name=excluded.resource_name, country_code=excluded.country_code, country_name=excluded.country_name, proxy_type=excluded.proxy_type, candidate_id=excluded.candidate_id, exit_ip=excluded.exit_ip, public_inbound_id=excluded.public_inbound_id, mixed_inbound_id=excluded.mixed_inbound_id, public_port=excluded.public_port, mixed_port=excluded.mixed_port, enabled=excluded.enabled, candidate_latency_ms=excluded.candidate_latency_ms, vless_latency_ms=excluded.vless_latency_ms, socks_latency_ms=excluded.socks_latency_ms, last_checked_at=excluded.last_checked_at, last_error_code=excluded.last_error_code, updated_at=excluded.updated_at`,
		value.ResourceName, value.CountryCode, value.CountryName, value.ProxyType, value.CandidateID, value.ExitIP, value.PublicInboundID, value.MixedInboundID, value.PublicPort, value.MixedPort, boolInt(value.Enabled), value.CandidateLatencyMS, value.VLESSLatencyMS, value.SOCKSLatencyMS, lastChecked, value.LastErrorCode, value.UpdatedAt.UTC().UnixMilli())
	return err
}

...
foreach ($token in @('--check', '--apply', '--public-origin', 'aimili-gateway.service', 'GATEWAY_CONFIG', 'aimili-gateway.db', 'mixedSourceCidrs', 'systemd-creds', 'LoadCredentialEncrypted', '/run/credentials/aimili-gateway.service/gateway-master-key')) {

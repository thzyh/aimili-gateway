...
const emit = defineEmits<{ copy: [row: ProxyGroupPayload]; check: [row: ProxyGroupPayload]; rotate: [row: ProxyGroupPayload] }>()
const typeLabel = (value: string) => value === 'residential' ? '住宅' : '机房'
const statusLabel = (value: string) => ({ ready: '可用', provisioning: '创建中', rotating: '换 IP 中', degraded: '异常', repair_required: '需修复', disabling: '回收中' }[value] ?? value)

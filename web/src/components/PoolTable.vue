      ...
      <tbody>
        <tr v-for="row in rows" :key="row.id" data-pool-row :data-row-id="row.id">
          ...
          <td class="row-actions">
            ...
            <button v-else-if="row.status === 'repair_required' || row.status === 'degraded'" :data-repair="row.id" class="icon-button activate-button" :disabled="busy !== ''" title="核对运行时身份、代理和当前公网协议" @click="emit('check', row)">重新检测并同步</button>
          </td>
        </tr>
      </tbody>
    </table>

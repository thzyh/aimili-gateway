    ...
    <table>
      ...
      <tbody>
        <tr v-for="row in rows" :key="row.id" data-pool-row :data-row-id="row.id">
          ...
          <td class="port-cell" :data-row-ports="row.id"><template v-if="row.egressSource === 'main' || row.slotNumber"><code>公网 {{ row.publicPort ?? row.vlessPort }}</code><small>mixed {{ row.mixedPort }}</small></template><span v-else>—</span></td>
...
.port-cell{display:flex;flex-direction:column;align-items:flex-start;justify-content:center;gap:2px}.port-cell small{color:var(--muted-text);font-size:10px}
</style>

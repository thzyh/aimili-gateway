...
import type { DedicatedStandbyPayload, ProtocolMode, ProxyGroupPayload } from '../api/client'
...
const emit = defineEmits<{ copy: [row: ProxyGroupPayload]; replace: [row: ProxyGroupPayload]; check: [row: ProxyGroupPayload]; protocol: [row: ProxyGroupPayload, mode: ProtocolMode]; 'standby-manual': [index: number] }>()
...
const standbyCountryText = (row?: DedicatedStandbyPayload) => row?.country ? countryDisplayName(row.country, [{ code: row.country, name: row.country }]) : '待分配国家'
...
<template>
  <div class="table-wrap">
    <table>
    <thead><tr><th>国家</th><th>IP 类型</th><th>出口 IP</th><th>端口</th><th>实测延迟</th><th v-if="protocol === 'vless'">公网协议</th><th>状态</th><th>专属热备用</th><th>最近检测</th><th class="actions-head">操作</th></tr></thead>
      <tbody>
...
.port-cell{display:flex;align-items:center}.standby-cell{min-width:210px}.standby-inline{display:grid;gap:3px;min-width:180px}.standby-state{width:max-content;padding:2px 6px;border-radius:999px;background:var(--subtle);color:var(--muted-text);font-size:10px;font-weight:750}.standby-state[data-state=ready]{background:rgba(24,174,112,.12);color:var(--healthy)}.standby-state[data-state=waiting_manual],.standby-state[data-state=degraded]{background:rgba(239,68,68,.12);color:var(--danger)}.standby-meta{overflow:hidden;color:var(--muted-text);font-size:11px;text-overflow:ellipsis;white-space:nowrap}.standby-manual{width:max-content;padding:3px 7px;border:1px solid var(--danger);border-radius:6px;background:transparent;color:var(--danger);font-size:10px;cursor:pointer}.standby-manual:disabled{opacity:.5;cursor:not-allowed}
</style>

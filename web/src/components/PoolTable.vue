...
const statusLabel = (value: string) => ({ standby: '待启用', ready: '可用', provisioning: '创建中', rotating: '换 IP 中', degraded: '异常', repair_required: '需修复', disabling: '回收中' }[value] ?? value)
    ...
    <table>
      ...
      <tbody>
        <tr v-for="row in rows" :key="row.id" data-pool-row>
          ...
          <td class="status-cell"><span class="status" :data-status="row.status"><i />{{ statusLabel(row.status) }}</span></td>

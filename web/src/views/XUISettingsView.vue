...
function makeNotice(kind: NoticeKind, title: string, message = ''): UiNoticeData {
  noticeSequence += 1
  return { id: `xui-notice-${noticeSequence}`, kind, title, message }
}
...
async function load(): Promise<void> {
  ...
  catch { notice.value = makeNotice('error', '3x-ui 信息读取失败', '3x-ui 维护信息暂时不可用。') }
}

async function run(path: string, action: string): Promise<void> {
  busy.value = action
  notice.value = action === 'repair'
    ? makeNotice('progress', '正在修复 3x-ui 受管资源', '只处理 agw- 命名空间，代理数据面保持受保护。')
    : makeNotice('progress', '正在检测 3x-ui 受管资源', '检测为只读操作。')
  try {
    data.value = await apiFetch<XUISettingsPayload>(path, { method: 'POST' })
    notice.value = action === 'repair'
      ? makeNotice('success', '3x-ui 受管资源修复完成', '已重新读取实际受管资源状态。')
      : makeNotice('success', '3x-ui 受管资源检测完成', '已重新读取实际受管资源状态。')
  }
  catch {
    notice.value = action === 'repair'
      ? makeNotice('error', '3x-ui 受管资源修复失败', '代理数据面保持不变，请查看运维记录。')
      : makeNotice('error', '3x-ui 受管资源检测失败', '检测失败，请稍后重试。')
  }
  finally { busy.value = '' }
}
...
async function openOriginal(): Promise<void> {
  busy.value = 'open'
  notice.value = makeNotice('progress', '正在进入 3x-ui 专家模式', '正在创建短期后台会话。')
  try {
    const destination = await openBackend('/api/v1/backends/3x-ui/login')
    if (destination) window.location.assign(destination)
  } catch { notice.value = makeNotice('error', '3x-ui 专家模式打开失败', '请使用 3x-ui 登录页手动登录。') }
  finally { busy.value = '' }
}
...
<template>
  <AppShell>
    <div class="subpage">
      ...
      <UiNotice v-if="notice" :key="notice.id" data-xui-notice class="page-notice" :notice="notice" @close="notice=null" />

...
import { apiFetch, idempotencyHeaders, type FreesubBackupPayload, type FreesubCandidatePayload } from '../api/client'
...
const candidates = ref<FreesubCandidatePayload[]>([])
const manualCandidateId = ref('')
let sequence = 0
...
const manualCandidates = computed(() => candidates.value.filter(candidate => candidate.candidateId !== backup.value?.candidateId))

function chooseManualDefault(): void {
  if (!manualCandidates.value.some(candidate => candidate.candidateId === manualCandidateId.value)) {
    manualCandidateId.value = manualCandidates.value[0]?.candidateId || ''
  }
}
...
async function load(): Promise<void> {
  try { backup.value = await apiFetch<FreesubBackupPayload>('/api/v1/freesub/backup'); chooseManualDefault() }
  catch { show('error', '备用连接读取失败', '无法读取 freesub 独立状态。') }
}

async function loadCandidates(): Promise<void> {
  try {
    const value = await apiFetch<FreesubCandidatePayload[]>('/api/v1/freesub/candidates')
    candidates.value = Array.isArray(value) ? value : []
    chooseManualDefault()
...
async function run(action: 'check' | 'replace' | 'manual-provision'): Promise<void> {
  ...
  try {
    const options: RequestInit = { method: 'POST', headers: idempotencyHeaders() }
    if (manual) options.body = JSON.stringify({ candidateId: manualCandidateId.value })
    backup.value = await apiFetch<FreesubBackupPayload>(`/api/v1/freesub/backup/${action}`, options)
    chooseManualDefault()
...
onMounted(async () => { await load(); await loadCandidates() })
</script>
...
<template>
  <AppShell>
    <section class="page">
      ...
      <UiNotice v-if="notice" :notice="notice" @close="notice = null" />
      <article v-if="backup" data-freesub-card class="backup-card" :data-status="backup.status">
        ...
        <dl>
          ...
          <div><dt>Ping0 风控</dt><dd>{{ backup.riskScore === undefined ? '未通过严格复核' : `风控 ${backup.riskScore}%` }}</dd></div>
          <div><dt>IP 属性</dt><dd>{{ backup.nativeIp ? (backup.nativeLabel || '原生 IP') : '未验证原生 IP' }}</dd></div>
        </dl>
        <div v-if="backup.scenarioStars" class="quality-scenes" data-quality-scenes>
          <span>TikTok {{ backup.scenarioStars.tiktok }}★</span>
          <span>跨境电商 {{ backup.scenarioStars.crossBorderEcommerce }}★</span>
          <span>社媒运营 {{ backup.scenarioStars.socialMedia }}★</span>
          <span>AI {{ backup.scenarioStars.ai }}★</span>
        </div>
        <p v-if="backup.status === 'waiting_manual'" class="manual">自动替换失败，等待人工处理。卡片与故障记录会继续保留。</p>
        <p class="repair-hint">单次自动修复只在真实故障后可用，并且每次故障最多执行一次。健康状态请使用下方“手动更换 IP”。</p>
        <div class="manual-switch">
          <label>合格候选
            <select v-model="manualCandidateId" data-manual-candidate :disabled="!!busy || manualCandidates.length === 0">
              <option v-if="manualCandidates.length === 0" value="">暂无其他合格候选</option>
              <option v-for="candidate in manualCandidates" :key="candidate.candidateId" :value="candidate.candidateId">
                {{ candidate.countryCode }} · {{ candidate.exitIp }} · {{ candidate.protocol.toUpperCase() }} · 风控 {{ candidate.riskScore }}%
              </option>
            </select>
          </label>
          <button data-manual-provision-freesub class="secondary" :disabled="!!busy || !manualCandidateId" type="button" @click="run('manual-provision')">{{ busy === 'manual-provision' ? '更换中' : '手动更换 IP' }}</button>
        </div>
        <div class="actions">
          <button data-check-freesub :disabled="!!busy || !backup.candidateId" type="button" @click="run('check')">{{ busy === 'check' ? '检测中' : '检测' }}</button>
          <button data-replace-freesub class="secondary" :disabled="!!busy || !canReplace" type="button" @click="run('replace')">{{ busy === 'replace' ? '修复中' : '执行单次自动修复' }}</button>
        </div>
      </article>
    </section>
  </AppShell>
...
.quality-scenes{display:flex;flex-wrap:wrap;gap:8px;margin:-6px 0 16px}.quality-scenes span{padding:6px 9px;border-radius:999px;background:color-mix(in srgb,var(--healthy) 10%,var(--panel));color:var(--healthy);font-size:12px;font-weight:750}.repair-hint{margin:12px 0;color:var(--muted-text);font-size:12px;line-height:1.6}.manual-switch{display:flex;align-items:end;gap:10px;margin:12px 0 18px;padding:12px;border:1px solid var(--border-soft);border-radius:10px;background:var(--subtle)}.manual-switch label{display:grid;flex:1;gap:6px;color:var(--muted-text);font-size:11px}.manual-switch select{height:38px;padding:0 10px;border:1px solid var(--border);border-radius:8px;background:var(--input);color:var(--text)}@media(max-width:700px){.manual-switch{align-items:stretch;flex-direction:column}}
</style>

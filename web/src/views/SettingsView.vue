...
onMounted(async () => {
  try {
    ...
    notice.value = messageFor(error, '高级设置暂时不可用')
  } finally {
    loading.value = false
  }
})
...
async function savePolicy(): Promise<void> {
  ...
  if (enabled.value && values.length === 0) {
    notice.value = '启用来源限制时至少填写一个具体 CIDR。'
    return
  }
  saving.value = true
  notice.value = '正在应用到所有已启用节点…'
  try {
    ...
    notice.value = saved.applyStatus === 'applied' ? 'SOCKS5H 来源策略已在全部在线节点生效。' : '策略已提交，正在等待应用结果。'
  } catch (error) {
    notice.value = messageFor(error, '来源策略应用失败')
  } finally {
    saving.value = false
  }
}
...
function messageFor(error: unknown, fallback: string): string {
  if (error instanceof APIError && error.message === 'repair_required') return `${fallback}：部分节点需要修复。`
  return `${fallback}。`
}
</script>
    ...
    <div v-else class="settings-layout">
      <section class="panel policy-panel">
        ...
        <form data-policy-form class="policy-form" @submit.prevent="savePolicy">
          ...
          <div class="form-footer"><span class="apply-state">当前状态：{{ policy?.applyStatus === 'applied' ? '已生效' : '待处理' }}</span><button :disabled="saving" type="submit">{{ saving ? '正在应用' : '保存策略' }}</button></div>
        </form>
      </section>

...
async function authorizeCurrentNetwork(): Promise<void> {
  saving.value = true
  notice.value = '正在仅授权当前网络…'
  try {
    const saved = await apiFetch<MixedSourcePolicyPayload>('/api/v1/settings/mixed-source-policy/authorize-current', { method: 'POST' })
    policy.value = saved
    enabled.value = saved.enabled
    cidrs.value = saved.cidrs.join('\n')
    notice.value = saved.applyStatus === 'applied' ? '当前网络已按单个地址授权，SOCKS5H 仍未对公网开放。' : '当前网络已提交授权，正在等待应用结果。'
  } catch (error) {
    notice.value = messageFor(error, '无法识别当前网络来源')
  } finally {
    saving.value = false
  }
}
